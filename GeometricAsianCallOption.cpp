#include "GeometricAsianCallOption.h"
#include "matlib.h"

Matrix GeometricAsianCallOption::payoff(const Matrix &prices) const {
  // TODO: implement when have time, along geoMean in matlib
  Matrix payoff;
  return payoff;
}

double GeometricAsianCallOption::price(const MultiStockModel &msm) const {
  ...
  return exp(-r * T) *
         (exp(-muHat * T) * S * normcdf(d1Hat) - K * normcdf(d2Hat));
}