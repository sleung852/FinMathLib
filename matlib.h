...
/*  Compute the geometric mean of a matrix's rows */
Matrix geoMeanRows(const Matrix &m);
...
/*  Create uniformly distributed random numbers */
Matrix randuniform(std::mt19937 &random, int rows, int cols);
/*  Create normally distributed random numbers */
Matrix randn(std::mt19937 &random, int rows, int cols);
...
void plot(const std::string &fileName, const Matrix &x, const Matrix &y);

/*  Plot a histogram */
void hist(const std::string &fileName, const Matrix &values,
          int numBuckets = 10);
...
double integralToInfinity(std::function<double(double)> f, double x,
                          int nSteps);

/*  Integrate f over the whole of R
by two applications of integral to Infinity */
double integralOverR(std::function<double(double)> f, int nSteps);
 ...
 */
double bisection(std::function<double(double)> f, double a, double b, double y,
                 double threshold);

double bisectionRecursion(std::function<double(double)> f, double a, double b,
                          double y, double threshold);

/*  Plot a histogram */
void hist(const std::string &fileName, const std::vector<double> &values,
          int numBuckets = 10);

/*  Create a line chart */
void plot(const std::string &fileName, const std::vector<double> &x,
