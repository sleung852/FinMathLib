...
std::ostream& operator<<(std::ostream& out, const PutOption& putOption ) {
    vector<string> params({"Maturity", "Strike"});
    vector<double> values({putOption.getMaturity(), putOption.getStrike()});
    out << putOption.describe(putOption.optionName, params, values);
    return out;
}
