...
std::ostream& operator<<(std::ostream& out, const UpAndOutOption& upAndOutOption ) {
    vector<string> params({"Maturity", "Strike", "Barrier"});
    vector<double> values({upAndOutOption.getMaturity(), upAndOutOption.getStrike(), upAndOutOption.getBarrier()});
    out << upAndOutOption.describe(upAndOutOption.optionName, params, values);
    return out;
}
