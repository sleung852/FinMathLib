...
std::ostream& operator<<(std::ostream& out, const DownAndOutOption& downAndOutOption ) {
    vector<string> params({"Maturity", "Strike", "Barrier"});
    vector<double> values({downAndOutOption.getMaturity(), downAndOutOption.getStrike(), downAndOutOption.getBarrier()});
    out << downAndOutOption.describe(downAndOutOption.optionName, params, values);
    return out;
}
