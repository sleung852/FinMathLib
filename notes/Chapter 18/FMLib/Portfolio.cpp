...
public:
    ...
    double priceByMonteCarlo( const MultiStockModel& model ) const;   
...
double PortfolioImpl::priceByMonteCarlo( const MultiStockModel& model ) const {
    double price = 0;

    map<double, vector<shared_ptr<ContinuousTimeOption> > > securitiesByMaturity;
    map<double, vector<double> > weightsByMaturity;
    map<double, set<string> > dependentStocksByMaturity;

    set<double> uniqueMaturities;
    // group by maturities
    for (int i=0; i<securities.size(); i++) {
        double maturity = securities[i]->getMaturity();
        securitiesByMaturity[maturity].push_back(securities[i]);
        weightsByMaturity[maturity].push_back(quantities[i]);
        uniqueMaturities.insert(maturity);
        dependentStocksByMaturity[maturity].insert(
            securities[i]->getStocks().begin(),
            securities[i]->getStocks().end());
    }
    // price each group of securities (options)
    auto itMaturity = uniqueMaturities.begin(); 
    while ((itMaturity != uniqueMaturities.end())) {
        BasketOfOptions bOptions(securitiesByMaturity[*itMaturity],
                                weightsByMaturity[*itMaturity],
                                *itMaturity,
                                dependentStocksByMaturity[*itMaturity]);
        price += bOptions.price(model);
        itMaturity++;
    }
    return price;
}
...
// void testMultiStockPortfolioPriceMonteCarlo() {
// 	auto model = MultiStockModel::createTestModel();
// 	auto p = Portfolio::newInstance();
// 	auto stocks = model.getStocks();
...
// 	double expected = p->price(model);
//     double actual = p->priceByMonteCarlo(model);
...
void testPortfolio() {
    ...
    // TEST( testMultiStockPortfolioPriceMonteCarlo );
