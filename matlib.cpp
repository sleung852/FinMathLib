...
double bisection(function<double(double)> f, double a, double b, double y,
                 double threshold) {
  double fa = f(a);
  double fb = f(b);
  while (abs(y - fa) > threshold || abs(y - fb) > threshold) {
    double mid = (a + b) / 2;
    double fmid = f(mid);
    if (fa < y && y < fmid) {
      b = mid;
...
double bisectionRecursion(function<double(double)> f, double a, double b,
                          double y, double threshold) {
  double fa = f(a);
  double fb = f(b);
  double mid = (a + b) / 2;
  if (abs(y - fa) < threshold || abs(y - fb) < threshold) {
    return mid;
  }
  // recursive call
  double fmid = f(mid);
 ...
 *  Convenience method for generating plots
 */
void plot(const string &file, const vector<double> &x,
